...
layout: docs
title: RTL
description: Learn how to enable support for right-to-left text in Snowdrop across our layout, components, and utilities.
...
We recommend getting familiar with Snowdrop first by reading through our [Getting Started Introduction page]({{< docsref "/getting-started/introduction" >}}). Once you've run through it, continue reading here for how to enable RTL.
...
There are two strict requirements for enabling RTL in Snowdrop-powered pages.
...
Our approach to building RTL support into Snowdrop comes with two important decisions that impact how we write and use our CSS:

1. **First, we decided to build it with the [RTLCSS](https://rtlcss.com/) project.** This gives us some powerful features for managing changes and overrides when moving from LTR to RTL. It also allows us to build two versions of Snowdrop from one codebase.
